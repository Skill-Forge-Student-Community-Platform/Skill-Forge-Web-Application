...
const ExploreDetails = ({userId, user}) => {
  ...
  const handleAddSave = async (e) => {
    try {
      ...
      if (response.status === 201) {
        ...
        setSavedEvents((prevSaved) => [...prevSaved, { eventId: e.id }]);
      } else {
  ...
  const handleWhatsapp = () => {
    if (!organizerProfile || typeof organizerProfile.getContactInfo !== 'function') {
    ...
    return (
      <div className="mt-6 pt-5 border-t border-gray-200">
        ...
        <div className="flex items-center">
          ...
          <div>
            <p className="font-medium text-gray-900">{organizerProfile.fullName}</p>
            <p className="text-sm text-gray-500">{organizerProfile.getOccupation() || "Event Organizer"}</p>
            {phoneNumber && (
              <p className="text-xs text-indigo-600 flex items-center mt-1">
                <FaPhoneAlt className="mr-1" /> {phoneNumber}
              </p>
            )}
          </div>
        </div>
