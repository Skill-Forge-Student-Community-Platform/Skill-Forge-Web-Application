...
export const useTeamStore = create((set) => ({
  ...
  createTeam: async (name, technology) => {
    try {
      const res = await axiosIntance.post("/teams/createTeam", {
        name,
        technology,
      });
      set((state) => ({
        ...
        invites: state.invites.filter(
          (invite) => invite.teamId !== res.data.team._id
        ), // Remove pending invites if relevant
      }));
  ...
  fetchTeamsByTechnology: async (technology) => {
    try {
      const response = await axiosIntance.get(
        `/teams/getTeamsByTechnology?technology=${technology}`
      );
      set({ teams: response.data });
    } catch (error) {
      console.error(
        "Error fetching teams by technology:",
