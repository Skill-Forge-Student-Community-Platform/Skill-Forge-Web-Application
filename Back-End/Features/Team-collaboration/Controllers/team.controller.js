...
export const sendInvite = async (req, res) => {
  ...
  try {
    ...
    if (team.creator.toString() !== req.user.id.toString()) {
      return res
        .status(403)
        .json({ error: "Only the team creator can send invites" });
    }
    ...
    res
      .status(200)
      .json({ message: `Invitation sent to ${userId} successfully!` });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
export const getReceivedInvites = async (req, res) => {
  ...
  try {
    const teams = await Team.find({ invites: userId })
      .populate("creator", "Username email")
      .select("name technology creator");
...
export const getSentInvites = async (req, res) => {
  const userId = req.user.id;
  try {
    const teams = await Team.find({ creator: userId }).populate(
      "invites",
      "Username email"
    );
...
export const kickMemberFromTeam = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      team.creator.toString() !== userId.toString()
    ) {
      return res
        .status(403)
        .json({
          error:
            "Only the team creator can remove members, or a member can leave voluntarily",
        });
    }
    ...
    if (memberId.toString() === team.creator.toString()) {
      return res
        .status(403)
        .json({ error: "Team creator cannot leave the team" });
    }
...
export const getTeamsByTechnology = async (req, res) => {
  ...
  try {
    const teams = await Team.find({
      ...
      .populate("members", "Username email"); // Populate members with their Username and email;

    if (!teams.length) {
      return res
        .status(404)
        .json({ error: "No teams found with the specified technology" });
    }
